...
class WebBotParser:
    ...
    ### provide text and news queries for Google and others
    ...
    def google_metadata(self, soup, file):
    ...
    def duckduckgo_metadata(self, soup, file):
        file = pathlib.Path(file).name
        _result_type = file.split('_')[-5]
        _engine = file.split('_')[0].split('/')[-1]
        _query = file.split('_')[1]
    ...
    def __init__(self, engine = None, queries = [], result_selector = '', metadata_extractor = lambda soup, file: {}):
        if engine is None and len(queries) == 0 and len(metadata_extractor(None, None)) == 0:
            raise Exception("To use the WebBotParser, either specify one of the supported engines,\
 or specify your own list of queries and/or a metadata extractor function.\
\nThe currently supported engines and result types are are: 'Google Text', 'Google News', 'DuckDuckGo Text', and 'DuckDuckGo News'.")
        if engine is not None:
            if engine == 'Google Text':
                self.result_selector = self.google_text_result_selector
                self.queries = self.google_text_queries
                self.metadata_extractor = self.google_metadata
            elif engine == 'Google News':
                self.result_selector = self.google_news_result_selector
                self.queries = self.google_news_queries
                self.metadata_extractor = self.google_metadata
            elif engine == 'DuckDuckGo Text':
                self.result_selector = self.duckduckgo_text_result_selector
                self.queries = self.duckduckgo_text_queries
                self.metadata_extractor = self.duckduckgo_metadata
            elif engine == 'DuckDuckGo News':
                self.result_selector = self.duckduckgo_news_result_selector
                self.queries = self.duckduckgo_news_queries
                self.metadata_extractor = self.duckduckgo_metadata
            else:
